      ...
      if (reader.files.length >= 2) {
        ...
        if (setting == "ask") {
          const r = await vscode.window.showQuickPick(["Yes", "No"], {
            canPickMany: false,
            placeHolder:
              "Do you wish to seperate the JSON into multiple files?",
          });
...
class DataClassGenerator {
  ...
  generateDataClazzes() {
    ...
    for (let clazz of this.clazzes) {
      ...
      if (insertConstructor) this.insertConstructor(clazz);
  ...
  insertConstructor(clazz) {
    ...
    if (clazz.constr != null) {
      if (clazz.constr.trimLeft().startsWith("const")) constr += "const ";
  ...
  insertCopyWith(clazz) {
    var hasNullables = false;
    let method = clazz.type + " copyWith({\n";
    for (const prop of clazz.properties) {
      if (prop.isNullable) {
        if (!hasNullables) hasNullables = true;
        method += `  ValueGetter<${prop.type}?>? ${prop.name},\n`;
      } else {
    ...
    for (let p of clazz.properties) {
      if (p.isNullable) {
        method += `    ${p.name}: ${p.name} != null ? ${p.name}?.call() : this.${p.name},\n`;
      } else {
  ...
  insertFromMap(clazz) {
    ...
    function customTypeMapping(prop, value = null) {
      ...
      value =
        value == null ? `${addLeftDefault}map['` + prop.jsonName + "']" : value;
    ...
    for (let p of props) {
      ...
      if (p.isEnum) {
        ...
        if (p.isPrimitive) method += customTypeMapping(p) + ` as ${p.type}`;
        else method += customTypeMapping(p);
      }
  ...
  insertEquatable(clazz) {
    ...
    const isHasNullable = props.find((p) => p.isNullable) != undefined;
    const split = short ? ", " : ",\n";
    let method = "@override\n";
    method += `List<Object${isHasNullable ? "?" : ""}> get props ${
      !short ? "{\n" : "=>"
    }`;
            ...
            for (let i = 0; i < words.length; i++) {
              ...
              if (word.length > 0 && word != "}" && word != "{") {
                ...
                if (word != "final" && word != "const") {
                  ...
                  if (isVariable) {
                    if (name == null) name = removeEnd(word, ";");
                  } else {
...
class JsonReader {
  ...
  async commitJson(progress, seperate) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (seperate) this.addGeneratedFilesAsImport(generator);
...
function getReplaceEdit(values, imports = null, showLogs = false) {
  ...
  if (ignoreComment == "")
    ignoreComment = "public_member_api_docs, sort_constructors_first";
  const lines = [];
  const ignores = [];
...
function readSetting(key) {
  return vscode.workspace
    .getConfiguration()
    .get("dart_data_class_generator." + key);
}
